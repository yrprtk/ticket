...
module.exports = {
  getUsers: async (req, res) => {
    try {
      let getObj = {};
      if(req.query.role){getObj['role']=req.query.role};
      const [{ users, usersCount }] = await User.aggregate([{
        $facet: {
          users: [ { $match: getObj }, { $skip: req.query.skip }, { $limit: req.query.limit } , { $sort: {created: req.query.sort} } ],
  ...
  patchUser: async (req, res) => {
    try {
      utils.deletePropFromObj(req.body, ['created', 'updated']);
      res.status(200).send(await User.updateOne({_id: req.params.userId}, req.body, {runValidators: true}));
    } catch (error) {
